...
static int devhost_cmdline(int argc, char** argv) {
    ...
    if (!strcmp(argv[1], "root")) {
        ...
        if ((status = device_create("root", NULL, &root_ops, &root_driver, 0, NULL, &dev)) < 0) {
            printf("devhost: cannot create root device: %d\n", status);
            return -1;
        }
        the_root_device = dev;
        if ((status = device_create("misc", NULL, &root_ops, &root_driver,
                                    MX_PROTOCOL_MISC_PARENT, NULL, &the_misc_device)) < 0) {
        ...
        if (argc < 4) return -1;
        if ((status = device_create("soc", NULL, &root_ops, &root_driver,
                                    MX_PROTOCOL_SOC, NULL, &dev)) < 0) {
        ...
        if ((status = device_create("acpi", NULL, &root_ops, &root_driver,
                                    MX_PROTOCOL_ACPI_BUS, NULL, &dev)) < 0) {
